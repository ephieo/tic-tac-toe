...
describe Game do
  ...
  let(:game_strings) { GameStrings.new }
  let(:io) { InputOutput.new(board.location, game_strings) }
  let(:rules) { GameRules.new(io, game_strings) }
  ...
  context 'testing that the player can finish the game' do
    it "if the game board is full a 'Game Over' string should be output to the terminal " do
      ...
      expect do
        game.start_game(board, InputOutput.new(board.location, game_strings))
  ...
  it ' If input isn\'t within the range of 0-8 it should return Incorrect string' do
    ...
    expect do
      game.start_game(Board.new(locations), InputOutput.new(board.location, game_strings))
